            ...
            for (int x = 0; x < InputImage.Width; x++)
            {
                for (int y = 0; y < InputImage.Height; y++)
                {
                        ...
                        else
                        {
                            ...
                            int grad = 0;
                            grad = (int)Math.Sqrt(Math.Pow(Math.Sqrt(g_x_0 * g_x_0 + g_y_0 * g_y_0),2)+Math.Pow(Math.Sqrt(g_x_1 * g_x_1 + g_y_1 * g_y_1),2)+ Math.Pow(Math.Sqrt(g_x_2 * g_x_2 + g_y_2 * g_y_2),2));
                            if (grad>T)
